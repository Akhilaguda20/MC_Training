...
class TestListAllPayrollsRoute:
    def test_returns_empty_list_when_no_records(self, client):
        http, _ = client
        response = http.get("/api/v1/payroll")
        assert response.status_code == 200
        assert response.json() == []

    def test_returns_all_payroll_records(self, client):
        http, repo = client
        repo.create("p-1", {"userId": "u-1", "baseSalary": "5000", "effectiveDate": "2026-01-01"})
        repo.create("p-2", {"userId": "u-2", "baseSalary": "6000", "effectiveDate": "2026-06-01"})
        ...
        data = response.json()
        assert len(data) == 2
        ids = {r["payrollId"] for r in data}
        assert ids == {"p-1", "p-2"}


class TestPayrollServiceListAll:
    def test_returns_all_payroll_records(self):
        repo = InMemoryPayrollRepository()
        service = make_service(repo=repo)
        service.create_payroll({"payrollId": "p-1", "userId": "u-1", "baseSalary": "5000", "effectiveDate": "2026-01-01"})
        service.create_payroll({"payrollId": "p-2", "userId": "u-2", "baseSalary": "6000", "effectiveDate": "2026-01-01"})

        result = service.list_all_payrolls()

        assert len(result) == 2

    def test_returns_empty_list_when_no_records(self):
        service = make_service()
        assert service.list_all_payrolls() == []
